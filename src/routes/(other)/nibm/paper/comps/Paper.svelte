<script lang="ts">
	let { paper }: { paper: NIBM_Paper } = $props();
	...
	>
		<div class="flex flex-col gap-2">
			<div class="flex items-center w-full">
				<img
					src={paper.thumbnail}
					class="rounded-lg w-full h-48 sm:h-64 object-cover object-top"
					alt=""
				/>
			</div>
			<div class="flex items-center w-full">
				<h1 class="text-sm sm:text-base m-0">{paper.title}</h1>
			</div>
		</div>
		<div class="flex items-center w-full mt-4 gap-2">
			<a
				href={paper.url}
				target="_blank"
				class="view-button text-center p-2 w-full text-xs sm:text-sm md:text-base m-0">View</a
			>
			<a
...
<style>
	...
	.view-button,
	.download-button {
		background: linear-gradient(135deg, var(--paper-view-grad, #2e2e2e9f), #121212);
		color: #f9f9f9;
	...
	.download-button {
		width: 3rem;
		cursor: pointer;
		background: linear-gradient(135deg, var(--paper-download-grad, #3e3e3e9f), #121212);
	}
